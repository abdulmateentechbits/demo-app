...
//
type LocaleListner = (locale:string)=>void

const localeListners:LocaleListner[] = []

export const addLocaleListener = (listener: LocaleListner) => {
  localeListners.push(listener)
}

export const removeLocaleListener = (listener: LocaleListner) => {
  const idx = localeListners.indexOf(listener)
  if (idx >= 0) localeListners.splice(idx, 1)
}

export const setLocale = (locale:string)=>{
  i18n.locale = locale
  localeListners.forEach(listener=>listener(locale));
}
